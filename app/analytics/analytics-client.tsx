      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm flex items-center justify-between">
          <div className="space-y-2">
            ...
            <div className="text-2xl font-bold">${data.grossRevenue.toFixed(2)}</div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm flex items-center justify-between">
          <div className="space-y-2">
            ...
            <div className="text-2xl font-bold">${data.inventoryValuation.toFixed(2)}</div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm flex items-center justify-between">
          <div className="space-y-2">
            ...
            <div className="text-2xl font-bold">{data.totalOrders} Orders</div>
        ...
        <div className="lg:col-span-2 bg-white border border-gray-200 rounded-xl p-6 shadow-sm space-y-6">
          <div className="flex items-center justify-between border-b pb-4">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg font-semibold">Monthly Revenue Growth</h2>
            </div>
            <span className="text-xs font-medium text-gray-400 uppercase">USD ($)</span>
          </div>

          <div className="flex items-end justify-between h-64 pt-4 px-2">
            {data.monthlyRevenue.map((item, index) => {
              ...
              return (
                <div key={index} className="flex flex-col items-center gap-3 flex-1 group">
                  ...
                  />
                  <span className="text-xs font-medium text-gray-500 mt-1">{item.month}</span>
                </div>
              )
            })}
          </div>
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm space-y-4">
            ...
            <div className="divide-y divide-gray-100 text-xs">
              {data.topProducts.map((product, index) => (
                <div key={index} className="flex justify-between py-2.5 first:pt-0 last:pb-0">
                  <div>
                    <div className="text-gray-700 font-medium">{product.name}</div>
                    <div className="text-gray-400 font-mono">{product.sku}</div>
                  </div>
                  <div className="flex items-center gap-2">
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-6 shadow-sm space-y-4">
              ...
              <div className="divide-y divide-amber-100 text-xs">
                {data.lowStockProducts.map((product, index) => (
                  <div key={index} className="flex justify-between py-2.5 first:pt-0 last:pb-0">
                    <div>
                      <div className="text-amber-800 font-medium">{product.name}</div>
                      <div className="text-amber-600 font-mono">{product.sku}</div>
                    </div>
